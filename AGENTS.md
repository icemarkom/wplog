...
| **Tab registration API** | `App.registerTab(navId, screenId, opts)` registers nav/screen tab pairs with wplog's tab manager. Options: `requiresGame` (disable when no game), `onActivate(game)` (callback). `_updateNavDisabled()` centralizes disabled-state management for all tabs. `_activateTab()` handles activation with session persistence. Eliminates timing races between integrating shells and `App.init()`. Issue [#226](https://github.com/icemarkom/wplog/issues/226). |
...
- **Hardware period sync**: `ClockEngine.getPeriod()` maps raw hardware period integers (1-4 regulation, 5=OT1, 6=OT2, etc., <=0 non-playing) to wplog period keys. `ClockEngine.applyPeriodToGame(game)` advances `game.currentPeriod` forward-only - never reverts - handling mid-game reconnect safely. Covers regulation and OT auto-advance; SO is wplog-internal and unaffected. `Events.refreshScoreBar()` is a new public method for external shells to repaint the period badge after a period change. The event modal's `_openModal()` now prefers the hardware-reported period (when valid and available) for the period pill pre-selection, so event entries land in the correct quarter automatically.
